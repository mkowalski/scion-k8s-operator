...
work with zero prior conversation history. Last updated: 2026-07-24.
...
## Repository inventory (all pushed, nothing local-only)

| Repo | Branch | State |
|---|---|---|
| github.com/mkowalski/scion-k8s-operator | `main` | v0.1.0 complete + live-e2e fixes merged. Operator, agent, registrar, CRD, manifests, OLM bundle, tests, docs. |
| github.com/mkowalski/metal3-dev-scripts (fork of openshift-metal3/dev-scripts) | `scion-topology` | `ENABLE_SCION_AS` feature: two-AS SCION topology on the hypervisor (9 podman containers, locally-built image, templated topologies, testcrypto trust material, discovery server, registrar). Fully validated live. NOT PR'd upstream (needs explicit approval). Plan: `docs/superpowers/plans/2026-07-24-scion-topology.md` in that repo, all tasks checked. |
| github.com/mkowalski/scion (fork of scionproto/scion) | `fix-sig-clearsession-panic` | Upstream fix, see below. |

Upstream filings:
- **Issue** [scionproto/scion#4953](https://github.com/scionproto/scion/issues/4953)
  — SIG nil-deref panic in `publishingRoutingTable.ClearSession` under
  remote-gateway churn. Root cause: `EngineController.run` closes the old
  routing table (nil-ing its route publisher) before closing the old engine
  whose router still processes session events.
- **PR** [scionproto/scion#4954](https://github.com/scionproto/scion/pull/4954)
  (head `mkowalski:fix-sig-clearsession-panic`, commit 416ee12) — teardown
  reorder + closed-state guard + tests. **ACTION: monitor for CI results and
  review feedback; bazel-based lints were not runnable locally.**
...
- **SCION topology LEFT RUNNING** in a sandbox, deliberately separate from
  ...
  - Known quirk: `scion-sig-b` could crash under heavy agent churn before
    the upstream fix; `podman start scion-sig-b` recovers. Rebuilding the
    image from the fix branch (or a release containing #4954) removes the
    crash.
...
1. Shepherd scionproto PR #4954 through review/CI.
2. Design + fix the OVN-K pod-egress bypass (spec-level change; brainstorm
   before coding).
3. Decide on upstream PR for the dev-scripts `scion-topology` branch.
4. CI for scion-k8s-operator (none exists; `.github/workflows` empty):
   build, vet, unit+envtest, discovery integration test, bundle-check.
5. Remaining items in `known-gaps.md` (API group placeholder, nonroot agent
...
- scionproto pinned at v0.15.0 everywhere (operator go.mod, dev-scripts
